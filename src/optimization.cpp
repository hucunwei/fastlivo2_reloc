                                         ...
                                         const PointCloudXYZRGB::Ptr& cloud)
{
    ...
    if (cloud && !cloud->empty()) {
        if (map_voxel_size_ > 0.0) {
            // ApproximateVoxelGrid stays valid on a full Livox scan. VoxelGrid's
            // 32-bit voxel index overflows when the scan spans more than ~130 m.
            ...
            voxel_filter.setLeafSize(static_cast<float>(map_voxel_size_),
                                     ...
                                     static_cast<float>(map_voxel_size_));
            voxel_filter.filter(*keyframe.cloud);
        } else {
...
void optimization::gpsHandler(const gnss_comm::GnssPVTSolnMsg::ConstPtr& gpsMsg)
{
    ...
    if (!first_gps) {
        ...
        map_origin_lat_ = gpsMsg->latitude;
        map_origin_lon_ = gpsMsg->longitude;
        map_origin_alt_ = gpsMsg->altitude;
    }
